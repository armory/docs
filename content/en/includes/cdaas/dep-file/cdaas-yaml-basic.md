...
manifests:
  # A directory containing multiple manifests. Armory CDaaS reads all yaml|yml files in the directory and deploys the manifests to the target defined in `targets`.
...
strategies:
  ...
  <strategyName1>:
    # The deployment strategy type. Armory CDaaS supports `canary`.
    canary:
