...
4. Add a **Query Template**. Use the following example:
   ...
   - The query contains variables that are automatically injected during canary analysis, but you must manually provide some of them during retrospective analysis. 
     - Time related variables like `promqlStepInterval` are automatically substituted by Borealis. For a full list, see [Retrospective Analysis]({{< ref "borealis-configuration-ui#retrospective-analysis" >}}).
     - `replicaSetName` needs to be set to the name of the ReplicaSet that Borealis created for this app version. It's used to differentiate between the current and next version of the app. Do this in the next step where you add Key/Value pairs. 
...
Once you have confidence in your queries, switching from manual approvals of canary steps to automatic approvals involves updating the `analysis` steps in your strategy. You can either comment out the 