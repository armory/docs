...
You can store secrets (and non secrets) in [supported secret stores]({{< ref "secrets" >}}) as well as in Kubernetes secrets if using the [Spinnaker Operator]({{< ref "armory-operator" >}}). This is the simplest route.
