...
- Strategy - Currently, only Canary is available as a deployment strategy. As Armory develops Project Aurora, more strategies can be added.
- Steps - These settings control how the your K8s deployment behaves. 
...
3. Add a stage to your pipeline with the following attributes:
  ...
  - **Stage Name**: Provide a descriptive name or leave the autogenerated name.
4. In **Account** field, select the target Kubernetes cluster you want to deploy to. This is a cluter where the Remote Network Agent and Argo Rollout is installed.
5. For **Manifest Source**, ensure that **Text** is selected.
6. For the **Manifest**, provide the following sample manifest that deploys NGINX but intentionally takes longer than usual. 
...
7. In the **How to Deploy** section, configure your canary steps. You can set percentage thresholds for how widely an app should get rolled out before it either waits for a manual approval or a set amount of time.
8. Save the pipeline and trigger a manual execution.
