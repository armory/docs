...
* Your Kubernetes cluster runs version 1.13 or later. If you do not have a cluster already, consult guides for [Amazon EKS](https://docs.aws.amazon.com/eks/latest/userguide/getting-started-console.html).
...
* You have created a persistent storage source for Armory Enterprise to use to store app settings and configured pipelines.
   * You can find a list of supported external storage providers for Armory Enterprise in the in the Compatibility Matrix's [External storage section]({{< ref "armory-platform-matrix#external-storage">}}).
