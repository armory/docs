...
- `namingStrategy`:
- `skin`:
- `onlySpinnakerManaged`: (V2 Only) When true, Spinnaker will only cache/display applications that have been created by Spinnaker.  Before placing in a "false" state, it is recommended to review the Kubernetes cluster configuration.  Once applied with a false flag, Spinnaker will analyze the cluster and automatically attempt to configure and populate applications for resources already present in Kubernetes, unless limited with `omitNamespaces`. It is important to note the [https://kb.armory.io/s/article/Autogenerated-Applications-Cannot-be-Deleted-w-onlySpinnakerManaged-set-to-false](increased possibilities of zombie applications in MySQL deployments).
- `debug`: true or false
- `dockerRegistries`:
